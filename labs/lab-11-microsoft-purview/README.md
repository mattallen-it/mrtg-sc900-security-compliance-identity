# Lab 11 - Microsoft Purview

> **Status:** Planned

## Primary Focus

Microsoft Purview Information Protection, sensitivity labels, Data Loss Prevention, retention, records management, Audit, eDiscovery, and related compliance capabilities.

## Documentation Standard

This lab will use the project master template located at [`/docs/lab-template.md`](../../docs/lab-template.md).

## AI Use Disclosure

AI tools may be used to support learning, troubleshooting, documentation organization, and technical review during this lab. All hands-on work, validation, screenshots, administrative decisions, and final documentation review will be completed by the project author. AI-generated guidance will be reviewed against Microsoft documentation and observed lab results before being accepted.

## Next Lab

`Lab 12 - MRTG SC-900 Security, Compliance, and Identity Capstone`
